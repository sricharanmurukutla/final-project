...
function ConfigurePage({ updateHomePageData }) {
  ...
  useEffect(() => {
    const fetchAllCategories = async () => {
      try {
        const response = await axios.get('http://159.203.85.6:3000/api/get-all-categories');
  ...
  const handleAllocationSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://159.203.85.6:3000/api/configure-budget', budgetData);
  ...
  const handleDeallocationSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://159.203.85.6:3000/api/deallocation-budget', deallocateData);
      console.log(response.data);
