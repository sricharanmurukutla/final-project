...
function HomePage() {
  ...
  const fetchData = (year, month, userId) => {
    axios
      .get(`http://159.203.85.6:3000/api/get-budgets/${year}/${month}/${userId}`)
      .then((response) => {
  ...
  const fetchTableData = () => {
    ...
    axios
      .get(
        `http://159.203.85.6:3000/api/get-table-data/${selectedYearTable}/${storedUserId}`
      )
      .then((response) => {
  ...
  const fetchUsedCategories = () => {
    axios
      .get("http://159.203.85.6:3000/api/get-all-categories")
      .then((response) => {
  ...
  const handleUsedBudgetSubmit = async (e) => {
    ...
    try {
      await axios.post("http://159.203.85.6:3000/api/enter-used-budget", payload);
  ...
  const fetchLineData = (userId, formattedStartMonth, formattedEndMonth) => {
    axios
      .get(
        `http://159.203.85.6:3000/api/get-line-graph-data-range/${usedselectedCategory}/${startYear}/${formattedStartMonth}/${endYear}/${formattedEndMonth}/${userId}`
      )
      .then((response) => {
